...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const existingClients = useAdminStore.getState().clients;
    
    // Check if Email or Phone already exists in the system
    const isDuplicate = existingClients.find(c => 
    ...
    try {
      ...
      try {
        useAdminStore.getState().addClient(newClient);
      } catch (err) {
        console.warn("Store update skipped");
      }

      // 3. SET LOCAL FLAG (To block future button clicks)
      if (selectedPlanId === 'TRIAL') {
